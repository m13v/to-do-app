...
export function parseMarkdownTable(markdown: string): Task[] {
  ...
  for (const line of lines) {
    ...
    if (inTable && line.startsWith('|') && line.endsWith('|')) {
      ...
      if (parts.length === 6) {
        // Old format: Category | Task | Status | Done
        tasks.push({
        ...
        // Intermediate format: Category | Task | Status | Done | Effort
        tasks.push({
          id: `${Date.now()}-${Math.random()}`,
          category: parts[1],
          task: parts[2],
        ...
        // New format: Category | Task | Status | Done | Effort | Criticality
        tasks.push({
          id: `${Date.now()}-${Math.random()}`,
          category: parts[1],
          task: parts[2],
...
export function tasksToMarkdown(tasks: Task[]): string {
  ...
  markdown += '| Category | Task | Status | Done | Effort | Criticality |\n';
  markdown += '|----------|------|--------|------|--------|-------------|\n';
  
  for (const task of tasks) {
    markdown += `| ${task.category} | ${task.task} | ${task.status} | ${task.done} | ${task.effort} | ${task.criticality} |\n`;
  }
