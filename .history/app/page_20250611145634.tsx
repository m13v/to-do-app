  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (storedMarkdown) {
        ...
        if (parsedTasks.some(t => !t.effort || t.effort === '' || !t.criticality || t.criticality === '')) {
          console.log('Saving migrated tasks with effort and criticality values');
          saveTasks(fullyMigratedTasks);
        }
      } else {
...
The Criticality column contains a number from 1 to 10 representing the criticality level of each task.
...
Your response will be parsed by the application, so it's critical to maintain the format.`;
  ...
  const handleAddTask = useCallback((afterId: string) => {
    setTasks(currentTasks => {
      ...
      const newTask: Task = { id: `${Date.now()}-${Math.random()}`, category, task: '', status: 'to_do', done: '', effort: '5', criticality: '2' };
