...
import { Loader2, Sparkles, Send, X, Copy, ArrowUpDown, ArrowUp, ArrowDown, Search } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { parseMarkdownTable, tasksToMarkdown, updateTask, deleteTask, Task } from '@/lib/markdown-parser';
...
export default function Home() {
  ...
  const handleDuplicateTask = useCallback(async (index: number) => {
    ...
    updatedTasks.splice(index, 0, duplicatedTask);
    setTasks(updatedTasks);
    await saveTasks(updatedTasks);
  }, [tasks, saveTasks]);

  const handleCellEdit = useCallback((index: number, field: 'category' | 'task') => {
  ...
  }, [editingCell, tasks, editValue, saveTasks]);
              ...
              <TableBody>
                <AnimatePresence>
                  {sortedTasks.map((task, index) => {
                    return (
                      <React.Fragment key={`task-${task.originalIndex}`}>
                        {/* Task row */}
                        <TableRow
                          onMouseEnter={() => setHoveredRow(task.originalIndex)}
                        ...
                        >
                          ...
                          <TableCell className="font-medium py-1 px-2">
                            {editingCell?.index === task.originalIndex && editingCell.field === 'category' ? (
                              <Input
                                value={editValue}
                              ...
                              <div
                                ...
                                onClick={() => handleCellEdit(task.originalIndex, 'category')}
                              >
                                {task.category}
                              </div>
                            )}
                          ...
                          <TableCell className="py-1 px-2">
                            {editingCell?.index === task.originalIndex && editingCell.field === 'task' ? (
                              <Textarea
                                value={editValue}
                                onChange={(e) => setEditValue(e.target.value)}
                                onBlur={handleSaveEdit}
                              ...
                              <div
                                ...
                                onClick={() => handleCellEdit(task.originalIndex, 'task')}
                                title={task.task}
                              >
                                {task.task.split('\n').map((line, i) => (
                          ...
                          <TableCell className="py-1 px-2">{task.status}</TableCell>
                          <TableCell className="py-1 px-2">{task.done}</TableCell>
                          <TableCell className="py-1 px-1 text-right">
                            <div className={`flex items-center justify-end gap-0.5 transition-opacity ${hoveredRow === task.originalIndex ? 'opacity-100' : 'opacity-0'}`}>
                              <Button
