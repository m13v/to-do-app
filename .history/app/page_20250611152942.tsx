...
export default function Home() {
  ...
  const saveTasks = useCallback(async (updatedTasks: Task[]) => {
    ...
    try {
      ...
      if (error) {
        // If the update fails, it might be because the user has no row yet.
        // Let's try to upsert. This is a common pattern.
        console.log('Update failed, trying to insert...');
        const { error: insertError } = await supabase
          .from('tasks')
          .insert({ user_id: session.user.id, content: markdown });
        if (insertError) {
          throw insertError;
        }
      }
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.content) {
        ...
        // No tasks found in Supabase, load from default file
        ...
        const fullyMigratedTasks = migrateTasksWithCriticality(migratedTasks);
        setTasks(fullyMigratedTasks);
        // And save it to Supabase for the new user
        await saveTasks(fullyMigratedTasks);
      }
    } catch (error) {
  ...
  }, [tasks, loading, session, saveTasks]);
