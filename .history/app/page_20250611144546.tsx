...
type SortField = 'id' | 'category' | 'task';
...
Your response will be parsed by the application, so it's critical to maintain the format.`;
  ...
  const sortedTasks = useMemo(() => {
    ...
    sortableTasks.sort((a, b) => {
      ...
      switch (sortField) {
        case 'id':
          // 'id' sort implies original order. We find the original index to compare.
          comparison = tasks.findIndex(t => t.id === a.id) - tasks.findIndex(t => t.id === b.id);
          break;
        case 'category':
