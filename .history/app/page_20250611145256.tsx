...
import { parseMarkdownTable, tasksToMarkdown, insertTaskAt, Task, migrateTasksWithEffort } from '@/lib/markdown-parser';
...
type SortField = 'id' | 'category' | 'task' | 'effort';
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (storedMarkdown) {
        ...
        // Save migrated tasks if migration occurred
        if (parsedTasks.some(t => !t.effort || t.effort === '')) {
          console.log('Saving migrated tasks with effort values');
        ...
        const migratedTasks = migrateTasksWithEffort(parsedTasks);
        setTasks(migratedTasks);
        localStorage.setItem('markdownContent', tasksToMarkdown(migratedTasks));
      }
    } catch (error) {
...
The table has the following columns: | Category | Task | Status | Done | Effort |
...
Your response will be parsed by the application, so it's critical to maintain the format.`;
  ...
  const handleAddTask = useCallback((afterId: string) => {
    setTasks(currentTasks => {
      ...
      const newTask: Task = { id: `${Date.now()}-${Math.random()}`, category, task: '', status: 'to_do', done: '', effort: '5' };
              ...
              <Table className="table-fixed w-full">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead 
                      className="w-[80px] cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800"
                      onClick={() => handleSort('effort')}
                    >
                      <div className="flex items-center gap-1">
                        Effort {getSortIcon('effort')}
                      </div>
                    </TableHead>
