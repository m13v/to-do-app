...
export default function Home() {
  ...
  useEffect(() => {
    return () => {
      if (lastMouseMoveTime.current) {
        cancelAnimationFrame(lastMouseMoveTime.current);
      }
    };
  }, []);
  ...
  }, [editingCell, editValue, tasks, saveTasks]);
  ...
  const handleStatusChange = useCallback(async (index: number, status: Task['status']) => {
    const updatedTask = {
      ...tasks[index],
      status
    };
  ...
  const handleDoneToggle = useCallback(async (index: number) => {
    const updatedTask = {
      ...tasks[index],
  ...
  const handleMouseMoveOnTable = useCallback((e: React.MouseEvent<HTMLTableElement>) => {
    ...
    lastMouseMoveTime.current = requestAnimationFrame(() => {
